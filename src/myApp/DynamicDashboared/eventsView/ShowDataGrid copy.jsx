...
import axiosInstance, { fetcher } from "src/myApp/utils/axios";
...
import { buildEventsApiUrl } from "src/myApp/DynamicSearch/services/SearchManager.api";
...
export default function ShowDataGrid({ date, location, rows1 }) {
  ...
  const parseTankActionSingleLine = (actionText) => {
    if (!actionText) return {};

    // تقسيم العملية على DOUBLE OPERATION
    const parts = actionText.split(/AND/i).map((p) => p.trim());

    const [op1Line, op2Line] = parts;

    const parseLine = (line) => {
      const regex = /^(\w+)\s+(BY|TO|FROM)\s+(.+?)(?:\s+\((OPEN|CLOSED)\))?$/i;
      ...
      if (match) {
        return {
          ...
          ValveStatus: match[4] ? match[4].toLowerCase() : "",
        };
      }
      return {};
    };

    const op1 = parseLine(op1Line || "");
    const op2 = parseLine(op2Line || "");
    ...
    return {
      ...
      isDoubleOperation: !!op2.TypeStatus,
    };
  };
